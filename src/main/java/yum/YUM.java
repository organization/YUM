...
public class YUM {
	...
	public static void UpdateCoreJAR(String downloadLink, String path, String repoVersion) {
		Server.getInstance().getScheduler().scheduleAsyncTask(new AsyncTask() {
			private String link, path, repoVersion;
			private boolean success = false;

			public AsyncTask setData(String link, String path, String repoVersion) {
				this.link = link;
				this.path = path;
				this.repoVersion = repoVersion;
				return this;
			...
			public void onCompletion(Server server) {
				if (!success) {
					YUMPlugin.getInstance().getLogger().info("* PASS: NUKKIT CORE #" + this.repoVersion);
					return;
				}
				YUMPlugin.getInstance().getLogger().info("* UPDATED: NUKKIT CORE #" + this.repoVersion);
				YUM.config.set("coreversion", Integer.valueOf(repoVersion));
			}
		}.setData(downloadLink, path, repoVersion));
	}
